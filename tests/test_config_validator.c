...
void test_validate_ChannelWithPdoEntry_ShouldAccept(void)
{
    static ecat_config_t config;
    baseline_config(&config);
    ...
    snprintf(ch->pdo_entry_index, sizeof(ch->pdo_entry_index), "0x6000");
    ch->pdo_entry_subindex = 1;
    TEST_ASSERT_EQUAL_INT(ECAT_CONFIG_OK, ecat_config_validate(&config));
}
