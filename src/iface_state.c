 ...
 *
 * Both are volatile NIC settings (not persisted across reboots by
 * the OS), so the only correctness concern is making sure a graceful
 * shutdown — and a *crashed* process's next start — both restore them
 * to the values that were in effect before the master was brought up.
 * That's what the `<state dir>/ecat_iface_<iface>.state` file is for:
...
#include "proc.h"
#include "config.h"   /* ecat_iface_validate */
...
static void capture_nic_settings(ecat_iface_state_t *s, edog_logger_t *logger)
{
    char output[2048];
...
static void apply_low_latency_nic(const char *iface, edog_logger_t *logger)
{
    ...
    /* Disable receive offloads that batch/merge packets */
    {
        ...
        if (ecat_run_argv("ethtool", argv, NULL, 0) == 0) {
            edog_log_info(logger, "%s: GRO/GSO/TSO offloads disabled", iface);
        }
    }
}

static void restore_nic_settings(const ecat_iface_state_t *s, edog_logger_t *logger)
{
...
{
    snprintf(buf, size, "%s/ecat_iface_%s.state", g_state_dir, iface);
}

static void persist_state(const ecat_iface_state_t *s, edog_logger_t *logger)
{
    ...
    if (rename(tmp, path) != 0) {
        edog_log_warn(logger, "Cannot rename %s -> %s: %s",
...
/*
 * Read <state dir>/ecat_iface_<iface>.state into @p s.  Returns true if
...
static void recover_from_crash(const char *iface, edog_logger_t *logger)
{
